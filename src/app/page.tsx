/* eslint-disable @typescript-eslint/no-explicit-any, react/no-unescaped-entities, react/jsx-no-comment-textnodes */
/* eslint-disable */
"use client";
...
declare global {
  interface Window {
    rrwebPlayer: new (options: {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      target: HTMLElement; // Reverted to HTMLElement to avoid any lint
      props: {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        events: any[];
        autoPlay?: boolean;
    ...
    }) => any; // Simplified type, replace 'any' with actual rrwebPlayer instance type if available
  }
}
...
function InstTerm({ step }: { step: number }) {
  const s = ISTEPS[step];
  return (
    <div>
...
const PLANS = [
  {
    ...
    btn: 'Start free', ent: 0, href: '/dashboard'
  },
  {
    ...
    btn: 'Get Builder', ent: 0, href: '/dashboard'
  },
  {
    ...
    btn: 'Get Studio', ent: 0, href: '/dashboard'
  },
  {
    ...
    btn: 'Contact us', ent: 1, href: 'mailto:hello@bugcatcher.app'
  },
];
    ...
    <>
      ...
      <nav className="nav">
        ...
        <div style={{ display: 'flex', gap: 9, alignItems: 'center' }}>
          ...
          <Link href="/dashboard" className="ncta" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>Start free</Link>
        </div>
      </nav>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }} className="hero-in">
              <div>
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', animation: 'up .6s .3s ease both', animationFillMode: 'both' }}>
                  <Link href="/dashboard" className="btn-r" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>Create free project</Link>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 1, background: 'var(--b)', border: '1px solid var(--b)', marginTop: 48 }}>
              {PLANS.map(pl => (
                <div key={pl.id} className={`pcard${pl.pop ? ' on' : ''}`} style={pl.ent ? { background: 'var(--bg2)', borderTop: '3px solid var(--p)' } : {}}>
                  ...
                  <Link href={pl.href || "/dashboard"} className="pbtn" style={{ textDecoration: 'none', display: 'inline-flex', justifyContent: 'center', ...(pl.ent ? { borderColor: 'rgba(167,139,250,.4)', color: 'var(--p)' } : {}) }}>{pl.btn}</Link>
                </div>
              ))}
            </div>
          ...
          <div className="si" style={{ padding: '0 40px 80px' }}>
            ...
            <div style={{ marginTop: 56, padding: 40, background: 'var(--bg2)', border: '1px solid var(--b)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', gap: 11, justifyContent: 'center', marginTop: 24, flexWrap: 'wrap' }}>
                <Link href="/dashboard" className="btn-r" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>Start building now</Link>
